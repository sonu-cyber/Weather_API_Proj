...
const arr_pressure = document.createElement('p');
...
const weather_api_res = async() => {
	try {
		const res = await fetch('https://api.openweathermap.org/data/2.5/weather?q=London,uk&APPID=93dde9f3522583d6a4a162552949e8fe&units=metric');
		...
		data_arr.forEach((arr, index, data_arr) => {
			...
			if(index === 1){
				for(let arr_obj in arr){
					...
					arr_description.textContent = `Description: ${arr[arr_obj].description} `;
			...
			if(index === 2){
			  arr_stations.classList.add('arr-stations');
				arr_stations.textContent = `Value: ${arr}`;
			}
			if(index === 3){
				...
				arr_pressure.classList.add('arr-pressure');
				arr_pressure.textContent = `Pressure: ${arr.pressure} `;
				console.log('arr.temp',arr.temp);
			}
			//index=4
			if(index === 4){
				arr_4.classList.add('arr-4');
				arr_4.textContent = `${arr}`;
			}
			//index=5
			if(index === 5){
				arr_speed.classList.add('arr-speed');
				arr_speed.textContent = `Speed: ${arr.speed} `;
				arr_deg.classList.add('arr-deg');
				arr_deg.textContent = `Deg: ${arr.deg}`;
			}
			if(index === 6){
				arr_all.classList.add('arr-all');
				arr_all.textContent = `All: ${arr.all}`;
			}
			if(index === 7){
				arr_7.classList.add('arr-7');
				arr_7.textContent = `Value: ${arr}`;
			}
			if(index === 8){
			...
			if(index === 9){
				...
				arr_9.textContent = `Value: ${arr }`;
				console.log(arr_9);
			}
			if(index === 10){
				arr_10.classList.add('arr-10')
				arr_10.textContent = `Value: ${arr }`;
				console.log(arr_10);
			}
			if(index === 11){
				arr_11.classList.add('arr-11')
				arr_11.textContent = `Value: ${arr }`;
				console.log(arr_11);
			}
			if(index === 12){
				arr_12.classList.add('arr-12')
				arr_12.textContent += `Value: ${arr }`;
				console.log(arr_12);
			}
			//Append to cards
			...
			card_3.append(arr_temp, arr_temp_min, arr_temp_max,arr_feels_like, arr_pressure);
			card_4.append(arr_4);
			card_5.append(arr_speed, arr_deg);
			...
			root.append(card_0, card_1, card_2, card_3, card_4, card_5, card_6, card_7, card_8, card_9, card_10, card_11, card_12);
